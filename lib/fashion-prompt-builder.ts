...
export function buildFashionPrompt(
  ...
  const itemList = analysis.items
    .map((item, i) =>
      isTr
        ? `${i + 1}. ${item.labelTr}: ${item.usageRuleTr}`
        : `${i + 1}. ${item.labelEn}: ${item.usageRuleEn}`
    )
    .join("\n");

  const fileNames = uploadedItems
    .map((item) => `${item.key}: ${item.fileName}`)
    .join("\n");
  ...
  if (isTr) {
    lines.push(
      ...
      "Her ürünün rengi, formu, dikişi, materyali, oranı ve karakteri korunmalı.",
      "Ürünleri yeniden tasarlama, başka ürünle değiştirme, logoları veya detayları bozma.",
      "Görsel gerçek fotoğraf hissi vermeli, yapay illüstrasyon gibi görünmemeli.",
      "Tam boy moda fotoğrafı üret.",
      "Kafa tamamen kadraj içinde olsun.",
      "Ayaklar tamamen kadraj içinde olsun.",
      "Eller, çanta ve takı crop ile kesilmesin.",
      "Kadraj biraz geniş olsun, modelin tüm silüeti rahatça sığsın.",
      "Doğal cilt, gerçek kumaş gerginliği, gerçek dikiş hissi ve ticari moda çekimi kalitesi istiyorum.",
      analysis.summaryTr,
    ...
    lines.push(
      "No cropped head, no cropped feet, no missing bag, no missing jewelry, no altered pants, no altered top, no missing outerwear, no product mutation, no fake fabric, no plastic skin, no distorted anatomy."
    );
    ...
    lines.push(
      ...
      "Preserve product fidelity at a very high level.",
      "Keep every item's exact color, silhouette, seams, materials, proportions, and visual identity.",
      "Do not redesign products, do not replace products, and do not alter details.",
      ...
      "Do not crop hands, bag, or jewelry.",
      "Use a slightly wider camera framing so the full silhouette fits comfortably.",
      "Natural skin, realistic fabric tension, believable stitching, and premium fashion-shoot quality are required.",
      analysis.summaryEn,
      "Uploaded item list:",
      itemList,
      "File names:",
